...
如果地图里只有一个 `4`，程序按单场比赛模拟。若地图里有两个 `4` 且最后一格仍为 `4`，程序按上下半场模拟：上半场以第一个 `4` 为终点，下半场保留上半场结束位置并改用最后一个 `4` 为终点。

可以用 `--runners` 指定 6 个普通参赛团子的编号：

```powershell
.\build-cuda\Release\wuwa_race_sim.exe --map .\data\map2.txt --runners 6,7,8,9,10,11 --sims 100000000 --cuda
```
